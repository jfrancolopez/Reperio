...
| N2 | Kernel read-only is set and verified before scanning | `hostd` `BLKROSET` + independent verification for whole disk and children (RPR-016) | RPR-016 mocked ioctl/open-identity tests; RPR-020 disposable acceptance attempts each write path outside privileged CI |
